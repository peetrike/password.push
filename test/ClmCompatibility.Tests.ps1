function Test-ClmCompatibility {
    ...
    .EXAMPLE
        Get-ChildItem C:\Scripts | Test-ClmCompatibility

        Scans each file in C:\Scripts and returns any issues that might occur in CLM.
    .NOTES
        Taken with love from https://github.com/FriedrichWeinmann/FredSnippets/blob/master/Test-ClmCompatibility.ps1
    .LINK
        https://learn.microsoft.com/powershell/module/microsoft.powershell.core/about/about_language_modes#constrainedlanguage-mode
    #>
    [CmdletBinding()]
    param (
            [Parameter(
    ...
    begin {
        ...
        function Find-PSCustomObject {
            ...
            $explanation = 'Custom object creation using PSCustomObject is not available in CLM. ' +
                'You can work around this issue by replacing it with "New-Object PSObject -Properties @{ ... }",' +
                ' which works in CLM.'
        ...
        function Find-MethodInvocation {
            ...
            $explanation = 'Cannot call methods on objects in CLM, other than ToString, ' +
                'unless the type is one of the few basic trusted types such as string or integer'
            Search-Ast -Ast $Ast -Type 'Method Invocation' -Explanation $explanation -Filter {
                if ($args[0] -isnot [Management.Automation.Language.InvokeMemberExpressionAst]) { return }
        ...
        function Find-TypeConversion {
            ...
            Search-Ast -Ast $Ast -Type 'Type Conversion' -Explanation $explanation -Filter {
                if ($args[0] -isnot [Management.Automation.Language.ConvertExpressionAst]) { return }
        ...
        function Find-AddType {
            ...
            Search-Ast -Ast $Ast -Type 'Add-Type' -Explanation $explanation -Filter {
                if ($args[0] -isnot [Management.Automation.Language.CommandAst]) { return }
        ...
        function Find-PropertyAssignment {
            ...
            $explanation = "Under CLM, assigning values to properties doesn't work, " +
                'unless the type is explicitly whitelisted by the engine. ' +
                'Generic PSObject objects - such as returned by ConvertFrom-Json or Import-Csv - ' +
                'ARE whitelisted however, so this scan may have a few false positives, sorry.'
            Search-Ast -Ast $Ast -Type 'Property Assignment' -Explanation $explanation -Filter {
                if ($args[0] -isnot [Management.Automation.Language.AssignmentStatementAst]) { return }
                if ($args[0].Left -isnot [Management.Automation.Language.MemberExpressionAst]) { return }
                if ($args[0].CommandElements | Where-Object Value -In $SafeTypes.FullName) { return }
